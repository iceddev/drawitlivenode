...
var rpcFunctions = {
  ...
  getWhiteBoard: function(id,userName){
    try{
      ...
      this.resultCB(whiteboards[id]);
    }catch(e){
      this.errorCB(e);
    }
  },
  getUsers: function(id){
    this.resultCB(whiteboards[id].users);
  },
  sendMessage: function(id,message,userName){
      ...
      this.resultCB({status:'ok',message: message});
  },
  square: function( num){
    try{
...
getSMD = function(){
  ...
  for(funcKey in rpcFunctions){
    smd.services[funcKey] = {};
  }
  return smd;
};
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('smd',function(rpc){
    ...
    var validChars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890".split();
